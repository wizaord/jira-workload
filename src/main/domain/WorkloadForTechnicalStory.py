...
class WorklogsForTechnicalStory:
    ...
    def extract_workloads_group_by_user_date_ts_in_csv_file(self, users: list[str] = None, date_debut: date = None, date_fin: date = None):
        technical_stories = self.jira_adapter.get_technical_stories()
        logger.info("technical_stories %s", technical_stories)
        self.__save_users_with_worklogs_in_csv_file(technical_stories, users, date_debut, date_fin)

    def __save_users_with_worklogs_in_csv_file(self, technical_stories: Issues, filtered_users: list[str] = None, date_debut: date = None, date_fin: date = None):
        ...
        for user_email in users_with_worklogs:
            ...
            for technical_story in technical_story_for_user.issues:
                ...
                for day, time_spent in spent_time_on_issue.items():
                    ...
                    if (date_debut is not None and day < date_debut) or (date_fin is not None and day > date_fin):
                        continue
                    ...
                    csv_content.append({
                        ...
                        'technical_status': technical_story.status
                    })
